module declaration
  ...
  integer                      dnyu,dnyv,dnyp
  integer                      nx,nz,ny0,nyF           ! Long penciles (second band): N(1,2)/2, N(2,2)/2, height of the box
  integer                      nyu0,nyuF,nyv0,nyvF ,nyp0,nypF          ! Long penciles (second band): N(1,2)/2, N(2,2)/2, height of the box 
  ...
  real*8,       allocatable :: spU(:,:,:),spX(:,:,:)   ! Variable to store spectra, temporal variable
  ...
  real(8),      allocatable :: Xmu (:),        XmCu (:,:,:)
  real(8),      allocatable :: Xmv (:),        XmCv (:,:,:)
  real(8),      allocatable :: Xmp (:),        XmCp (:,:,:)
  real(8),      allocatable :: Um (:),U2m (:),UmC (:,:,:),U2mC (:,:,:)
  real(8),      allocatable :: Vm (:),V2m (:),VmC (:,:,:),V2mC (:,:,:)
  real(8),      allocatable :: Wm (:),W2m (:),WmC (:,:,:),W2mC (:,:,:)
  real(8),      allocatable :: Pm (:),P2m (:),PmC (:,:,:),P2mC (:,:,:)
